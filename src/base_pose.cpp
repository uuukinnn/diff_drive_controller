...
void base_pose::joint_state_cb( const sensor_msgs::JointStatePtr &msg )
{
	...
	th += ( ( wheel_diam * dt * right_diff ) / ( 2.0 * wheel_base ) ) - ( ( wheel_diam * dt * left_diff ) / ( 2.0 * wheel_base ) );

	//generate twist
//	odom.twist.twist.linear.x = ( ( ( wheel_diam * vel_left ) / 4.0 ) + ( ( wheel_diam2 * vel_right ) / 4.0 ) ) * cos( th );
	odom.twist.twist.linear.x = ( ( ( wheel_diam * vel_left ) / 4.0 ) + ( ( wheel_diam2 * vel_right ) / 4.0 ) );
//	odom.twist.twist.linear.y = ( ( ( wheel_diam * vel_left ) / 4.0 ) + ( ( wheel_diam2 * vel_right ) / 4.0 ) ) * sin( th );
	odom.twist.twist.linear.y = 0;
	odom.twist.twist.angular.z = ( ( wheel_diam * vel_left ) / ( 2.0 * wheel_base ) ) - ( ( wheel_diam * vel_right ) / ( 2.0 * wheel_base ) );	
